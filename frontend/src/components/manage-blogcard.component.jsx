...
function BlogStats({stats}){
    return <div className="flex gap-2 max-lg:mb-6 max-lg:pb-4 border-grey max-lg:border-b">
        {
            Object.keys(stats).map((info, i)=>{
                return !info.includes("parent") ? <div className={"flex flex-col items-center w-full h-full justify-center p-4 px-6 "+ (i!=0 ? "border-grey border-l ":"")} key={i}>
...
export function ManagePublishedBlogCard({blog}) {
    ...
    const {blog_id, banner, title, publishedAt, activity} = blog
      ...
      <div className="flex gap-10 border-b mb-6 max-md:px-4 border-grey pb-6 items-center">
        ...
        <div className="flex flex-col justify-between py-2 w-full min-w-[300px]">
          <div>
            ...
            <p>Published on {getFullDay(publishedAt)}</p>
          </div>
...
export function ManageDraftBlogPost({blog}){
...
const deleteBlog = (blog, access_token, target) => {
  ...
  axios.post(import.meta.env.VITE_SERVER_DOMAIN + "/delete-blog",{blog_id},{
    ...
    setStateFunc(prev => {
      ...
      return {
        ...prev, totalDocs: totalDocs-1, deletedDocCount:deletedDocCount+1 
      }
    })
  }).catch(err=>{
    console.log(err)
